"""G3 U13 unit_test 검증 메타 정합화 (3.G.A.1·A.2)"""
import json, pathlib

ROOT = pathlib.Path(__file__).parent.parent
SRC  = ROOT / "backend/data/math/G3/U13_shapes/unit_test.json"

VERIFICATION = {
    "concept_source": "Go Math Grade 3 Ch.12 Unit Assessment 'Two-Dimensional Shapes'",
    "procedure_source": "EngageNY Grade 3 Module 7 End-of-Module Assessment — Geometry",
...
def normalize_problem(p: dict) -> dict:
    ...
    if "expected_errors" not in p:
        p["expected_errors"] = LESSON_TO_ERR.get(p.get("lesson_ref",""), ["3.G.A.1.M01"])
...
def upgrade():
    ...
    d["vertical_alignment"] = {
        "prerequisite": "G3 U13 L1~L7 — 도형 분류·분할 (3.G.A.1·A.2)",
        "current":      "G3 U13 unit_test — 도형 단원 평가",
        "successor":    "G4 — 다음 학년 (도형 더 깊이)",
    }
    d.setdefault("metadata", {})
    d["metadata"]["total_questions"] = len(problems)
    d["metadata"]["upgraded"] = True
    d["metadata"]["upgrade_script"] = "scripts/upgrade_u13_unit_test.py"
