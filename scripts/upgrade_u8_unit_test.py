"""
G3 U8 unit_test 7단계 마이그레이션 (검증 메타 추가)
==============================================================================
20문항. 표준: 3.NF.A.1 (단위분수), 3.NF.A.2 (수직선), 3.NF.A.3 (동치)
"""

import json, pathlib

ROOT = pathlib.Path(__file__).parent.parent
SRC  = ROOT / "backend/data/math/G3/U8_understand_fractions/unit_test.json"
...
def make_verification() -> dict:
    return {
        "concept_source": "Go Math Grade 3 Ch.8 Unit Assessment 'Understand Fractions'",
        "procedure_source": "EngageNY Grade 3 Module 5 End-of-Module Assessment — Fractions",
...
def normalize_problem(p: dict) -> dict:
    ...
    if "expected_errors" not in p:
        p["expected_errors"] = LESSON_TO_ERR.get(p.get("lesson_ref",""), ["3.NF.A.1.M01"])
...
def upgrade():
    ...
    d["vertical_alignment"] = {
        "prerequisite": "G3 U7 — 나눗셈 사실 전략 (3.OA.C.7)",
        "current":      "G3 U8 — 분수의 의미 단원 평가 (3.NF.A.1, 3.NF.A.2, 3.NF.A.3)",
        "successor":    "G3 U9 — 분수 비교 (3.NF.A.3d)",
    }
    d.setdefault("metadata", {})
    d["metadata"]["total_questions"] = len(problems)
    d["metadata"]["upgraded"] = True
    d["metadata"]["upgrade_script"] = "scripts/upgrade_u8_unit_test.py"
