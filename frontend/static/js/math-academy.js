...
function advanceMathStage() {
    ...
    if (s === 'unit_test') {
        ...
        // Fire-and-forget score submission to backend
        ...
        _renderRoundSummary({
            ...
            onContinue: () => {
                if (!passed) {
                    // Retry: reload the unit test
                    startMathUnitTest(mathState.grade, mathState.unit);
                } else {
                    // Return to math home / lesson selector
                    if (typeof exitMathLesson === 'function') exitMathLesson();
                    else if (typeof loadMathHome === 'function') loadMathHome();
                }
            },
        });
        return;
...
function _checkUnitTestAnswer(problemId, userAnswer) {
    ...
    const correctRaw = String(problem.correct_answer != null ? problem.correct_answer
                            : (problem.answer != null ? problem.answer : '')).trim();
