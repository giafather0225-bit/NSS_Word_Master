...
from backend.models import StudyItem, Progress, Reward, Schedule, UserPracticeSentence, Lesson, Word, WordReview
...
@app.post("/api/review/register-lesson")
def register_lesson_for_review(
    req: RegisterLessonReviewRequest,
    db: Session = Depends(get_db),
):
    """Register all words from a completed lesson into SM-2 review system."""
    study_items = (
        db.query(StudyItem)
        .filter(
            StudyItem.subject == req.subject,
...
def get_today_reviews(db: Session = Depends(get_db)):
    """Return all words due for review today."""
...
def submit_review_result(
    ...
    """Submit a review result and run SM-2 algorithm."""
...
def get_review_stats(db: Session = Depends(get_db)):
    """Overall SM-2 review statistics."""
    today_str = _date.today().isoformat()
