...
_REFINE_PROMPT_TMPL = """You are an English vocabulary extraction assistant for a kids' learning app.

Below is raw OCR text extracted from a scanned English vocabulary textbook page.
Extract ONLY the vocabulary entries.

Return a single valid JSON array. Each object MUST have exactly these keys:
- "word": the English vocabulary word (string)
- "pos": part of speech abbreviated (e.g. "n.", "v.", "adj.", "adv.") — use "" if unclear
- "definition": a short, clear English definition (one sentence, kid-friendly)
- "example": one short English example sentence using the word

Rules:
- Output ONLY the JSON array. No markdown fences, no explanations.
- Skip any text that is clearly not a vocabulary entry (page numbers, headers, etc.).
- If the same word appears multiple times, keep only the best entry.
...
async def refine_with_ollama(ocr_text: str, model: str | None = None) -> list[dict]:
    ...
    Args:
        ocr_text: Tesseract 등에서 나온 원시 텍스트
        model: Ollama 모델 이름 (기본: OLLAMA_OCR_MODEL)
    Returns:
        정제된 단어 목록
    Raises:
        ValueError: 파싱 실패
        httpx.HTTPError: Ollama 통신 오류
    """
    _model = model or OLLAMA_OCR_MODEL
    prompt = _REFINE_PROMPT_TMPL.format(ocr_text=ocr_text[:6000])  # 토큰 안전 제한
    ...
    payload = {
        ...
        "stream": False,
        "options": {"temperature": 0.1},  # 낮은 온도 → 일관된 JSON 출력
    }
    ...
    if not cleaned:
        raise ValueError("정제 결과에 유효한 단어 항목이 없습니다.")
